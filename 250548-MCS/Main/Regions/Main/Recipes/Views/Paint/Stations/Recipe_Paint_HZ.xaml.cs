...
namespace HMI.MainRegion.Recipes.Views
{
    ...
    [ExportView("Recipe_Paint_HZ")]
    public partial class Recipe_Paint_HZ
    {

        public Recipe_Paint_HZ()
        {
        ...
        private void to_ValueChanged(object sender, VisiWin.DataAccess.VariableEventArgs e)
        {
            if ((bool)to.IsChecked)
            {
                ...
                s.RawLimitMin = 180;
                s.RawLimitMax = 320;
                ul.RawLimitMax = s.Value + 60;
                ul.RawLimitMin = s.Value + 1;
                ll.RawLimitMax = s.Value - 1;
                ll.RawLimitMin = s.Value - 60;
            }
            else
            {
                s.RawLimitMin = 0;
