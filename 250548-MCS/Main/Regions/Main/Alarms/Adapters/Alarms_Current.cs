    ...
    public class Alarms_Current : AdapterBase
    {
        ...
        private void AckExecuted(object parameter)
        {
            Task taskA = Task.Run(() =>
            {
                ...
                ApplicationService.SetVariableValue("CPU2.PLC.Blocks.00 Main.02 HMI.00 Main.DB HMI Main.Alarm.from.Acknowledge", true);
            });
            taskA.ContinueWith(async x =>
            {
                ...
                ApplicationService.SetVariableValue("CPU2.PLC.Blocks.00 Main.02 HMI.00 Main.DB HMI Main.Alarm.from.Acknowledge", false);
