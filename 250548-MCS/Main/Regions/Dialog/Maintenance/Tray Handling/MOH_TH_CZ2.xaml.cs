        ...
        private void Reset_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            if (MessageBoxView.Show("@Maintenance.Text15", "@Maintenance.Text16", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                ...
                if (btn1.IsSelected)
                {
                    ...
                    Task taskA = Task.Run(() =>
                    {
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.04 Tray handling.07 CZ.02 Air 2.DB CZ Air 2 HMI.Actual.Circulating fan.Operating hours.Reset", true);
                    });
                    taskA.ContinueWith(async x =>
                    {
                        await Task.Delay(1000);
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.04 Tray handling.07 CZ.02 Air 2.DB CZ Air 2 HMI.Actual.Circulating fan.Operating hours.Reset", false);
                ...
                if (btn2.IsSelected)
                {
                    ...
                    Task taskA = Task.Run(() =>
                    {
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.04 Tray handling.07 CZ.02 Air 2.DB CZ Air 2 HMI.Actual.Cooler.Operating hours.Reset", true);
                    });
                    taskA.ContinueWith(async x =>
                    {
                        await Task.Delay(1000);
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.04 Tray handling.07 CZ.02 Air 2.DB CZ Air 2 HMI.Actual.Cooler.Operating hours.Reset", false);
