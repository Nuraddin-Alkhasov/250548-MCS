        ...
        private void Reset_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            if (MessageBoxView.Show("@Maintenance.Text15", "@Maintenance.Text16", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                ...
                if (btn1.IsSelected)
                {
                    ...
                    Task taskA = Task.Run(() =>
                    {
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.04 Tray handling.09 US 1.01 Belt.DB US 1 Belt HMI.Actual.Belt.Operating hours.Reset", true);
                    });
                    taskA.ContinueWith(async x =>
                    {
                        await Task.Delay(1000);
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.04 Tray handling.09 US 1.01 Belt.DB US 1 Belt HMI.Actual.Belt.Operating hours.Reset", false);
                ...
                if (btn2.IsSelected)
                {
                    ...
                    Task taskA = Task.Run(() =>
                    {
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.04 Tray handling.10 US 2.01 Belt.DB US 2 Belt HMI.Actual.Belt.Operating hours.Reset", true);
                    });
                    taskA.ContinueWith(async x =>
                    {
                        await Task.Delay(1000);
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.04 Tray handling.10 US 2.01 Belt.DB US 2 Belt HMI.Actual.Belt.Operating hours.Reset", false);
