...
namespace HMI.DialogRegion.MO.Views
{
    ...
    [ExportView("MO_HZ2_EF")]
    public partial class MO_HZ2_EF
    {
        public MO_HZ2_EF()
        {
            InitializeComponent();
        ...
        private void View_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            VPS1 = VS.GetVariable("CPU2.PLC.Blocks.04 Tray handling.06 HZ.02.02 Exhaust.DB HZ 2 Exhaust HMI.Actual.Purge.Pressure switch 1");
            VPS1.Change += VPS1_Change;

            VPS2 = VS.GetVariable("CPU2.PLC.Blocks.04 Tray handling.06 HZ.02.02 Exhaust.DB HZ 2 Exhaust HMI.Actual.Purge.Pressure switch 2");
        ...
        private void V_Off_Click(object sender, RoutedEventArgs e)
        {
            if (MessageBoxView.Show("@MachineOverview.Text53", "@Buttons.Text64", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                Task taskA = Task.Run(() =>
                {
                    ApplicationService.SetVariableValue("CPU2.PLC.Blocks.04 Tray handling.06 HZ.02.02 Exhaust.DB HZ 2 Exhaust HMI.PC.Drive.from.Restart", true);
                });
                taskA.ContinueWith(async x =>
                {
                    await Task.Delay(1000);
                    ApplicationService.SetVariableValue("CPU2.PLC.Blocks.04 Tray handling.06 HZ.02.02 Exhaust.DB HZ 2 Exhaust HMI.PC.Drive.from.Restart", false);
