    ...
    public class MO_Status_1 : AdapterBase
    {
        ...
        private void DeleteExecuted(object parameter)
        {
            if (MessageBoxView.Show("@Status.Text26", "@Status.Text27", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                ...
                Task taskA = Task.Run(() =>
                {
                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.from.Data.Delete", true);
                });
                taskA.ContinueWith(async x =>
                {
                    await Task.Delay(1000);
                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.from.Data.Delete", false);
                    CloseExecuted(null);
            ...
            if (MessageBoxView.Show("@Status.Text29", "@Status.Text28", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                Task taskA = Task.Run(async () =>
                {
                    if (Paints.Where(x => x.Header.Id.ToString() == pId).ToList().Count != 0) 
                    {
                        ...
                        VWVariable v = paint.VWRecipe.VWVariables.Where(x => x.Item.ToString() == "CPU1.PLC.Blocks.03 Basket coating.02 DT.00 Main.DB DT PD.Station.Baskets.Coatings").ToList().First();
                        ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Baskets.Coatings.Set", v.Value);
                        
                    }

                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.from.Data.Update", true);

                    await Task.Delay(1000);
                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.from.Data.Update", false);
                    CloseExecuted(null);
                });
        ...
        {
            ApplicationService.SetView("DialogRegion2", "Recipe_Selector", "MO_Status_1");
        }
        #endregion
        ...
        protected override void OnViewLoaded(object sender, ViewLoadedEventArg e)
        {
            if (sender.GetType().Name == e.View.GetType().Name)
            {
                ...
                SetData((uint)ApplicationService.GetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Header.MR"));
        ...
        void SetData(uint _mrId)
        {
            try
            {
                ...
                uint OrderId = (uint)ApplicationService.GetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Header.Order");
                if (OrderId > 0)
                {
                ...
                uint BoxId = (uint)ApplicationService.GetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Header.Box");
                if (BoxId > 0)
                {
                ...
                uint ChargeId = (uint)ApplicationService.GetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Header.Charge");
                if (ChargeId > 0)
                {
                ...
                uint RunId = (uint)ApplicationService.GetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Header.Layer");
                if (RunId > 0)
                {
        ...
        public void SetData(int _mrId)
        {
            ...
            if (_mrId > 0)
            {
                ...
                byte actualLayer = (byte)ApplicationService.GetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Charge.Layer.Actual");
                ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Header.MR", _mrId);
                ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Charge.Available", 1);
                ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Charge.Machine.Actual", 1);
                ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Charge.Layer.Set", counter);

                if (actualLayer < counter)
                {
                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Charge.Paint.Actual", actualLayer == 0 ? 0 : MR.Layers[actualLayer - 1].Paint.Header.Id);
                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Charge.Paint.Next", MR.Layers[actualLayer].Paint.Header.Id);
                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Charge.Machine.Next", MR.Layers[actualLayer].Machine);
                }
                else
                {
                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Charge.Paint.Actual", MR.Layers[counter - 1].Paint.Header.Id);
                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Charge.Paint.Next", 0);
                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.00 Main.02 HMI.01 PC.DB PC.State.to.Data.Charge.Machine.Next", 0);
                }
            }
        }
    } 
