...
namespace HMI.DialogRegion.MO.Views
{
    ...
    public partial class MO_Coating_SF
    {
        ...
        private void View_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            VPurge = VS.GetVariable("CPU1.PLC.Blocks.03 Basket coating.01 CD.07 Supply.DB CD Supply HMI.Actual.Purge.On");
            VPurge.Change += VPurge_Change;

            VPS1 = VS.GetVariable("CPU1.PLC.Blocks.03 Basket coating.01 CD.07 Supply.DB CD Supply HMI.Actual.Purge.Pressure switch 1");
            VPS1.Change += VPS1_Change;

            VPS2 = VS.GetVariable("CPU1.PLC.Blocks.03 Basket coating.01 CD.07 Supply.DB CD Supply HMI.Actual.Purge.Pressure switch 2");
        ...
        private void V_Off_Click(object sender, RoutedEventArgs e)
        {
            if (MessageBoxView.Show("@MachineOverview.Text53", "@Buttons.Text64", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                Task taskA = Task.Run(() =>
                {
                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.03 Basket coating.01 CD.07 Supply.DB CD Supply HMI.PC.Drive.from.Restart", true);
                });
                taskA.ContinueWith(async x =>
                {
                    await Task.Delay(1000);
                    ApplicationService.SetVariableValue("CPU1.PLC.Blocks.03 Basket coating.01 CD.07 Supply.DB CD Supply HMI.PC.Drive.from.Restart", false);
